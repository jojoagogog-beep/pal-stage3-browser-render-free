...
FAIL=re.compile(r'(入力してください|未入力|入力.{0,20}エラー|エラーがあります|必須(?:項目)?です|必須項目|正しく入力|入力内容.{0,20}(?:誤|エラー)|ご確認の上.{0,40}(?:修正|戻る)|required field|(?:phone(?:\s+number)?|telephone|mobile|address|postal(?:\s+code)?|postcode|zip).{0,30}(?:is\s+)?required|please.{0,30}(?:fill|enter|select|choose)|failed\s+to\s+send|unable\s+to\s+send|could\s+not\s+send|there\s+was\s+an\s+error.{0,60}send|validation error|unexpected\s+token.{0,120}not\s+valid\s+json|not\s+valid\s+json|invalid|submission.{0,24}(?:rejected|failed|declined)|comment.{0,40}could\s+not\s+be\s+posted|could\s+not\s+be\s+posted|exceeded\s+the\s+allowed\s+amount\s+of\s+characters|too\s+many\s+characters|character\s+limit|please\s+confirm\s+you(?:\s+are|\u2019re|\'re)\s+human|confirm\s+you(?:\s+are|\u2019re|\'re)\s+human|human\s+verification)',re.I)

DEMO_FORM=re.compile(r'(?:demo\s+form|this\s+demo\s+form|connect\s+it\s+to.{0,120}to\s+go\s+live|form\s+is\s+ready.{0,80}(?:go\s+live|connect))',re.I)
...
async def click_and_evidence(page,loc,message,email,before_text):
 ...
 corr2xx=any(x['matches_form_payload'] and 200<=x['status']<300 for x in responses);corr_fail=any(x['matches_form_payload'] and 400<=int(x['status'])<600 for x in responses)
 def pathmatch(rx,u):
...
async def process_task(browser,t):
 ...
 try:
  ...
  if DEMO_FORM.search(txt):return {**out,'outcome':'CONFIRMED_NOT_SENT','reason':'DEMO_FORM_NOT_LIVE','evidence':{'pre_submit':True}}
